...
class BinhDuongServiceV2 {
    ...
    async getAllStations() {
        try {
            // DEBUG: Log V2 service usage
            console.log('🔍 DEBUG: V2 Service getAllStations called');

            const metadata = await StationMetadataV2.find({}).sort({ name: 1 });
            console.log('🔍 DEBUG: V2 metadata count:', metadata.length);

            const currentDataList = await CurrentDataV2.find({});
            console.log('🔍 DEBUG: V2 current data count:', currentDataList.length);
    ...
    async getStationHistory(stationKey, start, end) {
        try {
            // DEBUG: Log V2 service history query
            console.log('🔍 DEBUG: V2 Service getStationHistory called for:', stationKey);
            ...
            const history = [];
            const seenTimestamps = new Set(); // Deduplicate by timestamp
            ...
            buckets.forEach(bucket => {
                bucket.measurements.forEach(measurement => {
                    ...
                    if (measurementTime >= startDate && measurementTime <= endDate) {
                        // Skip if already seen this timestamp
                        if (seenTimestamps.has(measurement.timestamp)) {
                            return;
                        }

                        // Skip if no data or empty data
                        if (!measurement.data || Object.keys(measurement.data).length === 0) {
                            return;
                        }
